...
async function createShareLink(request: Request, response: Response, next: NextFunction) : Promise<void> {
	...
	const scriptShareLink: ScriptShareLink = new ScriptShareLink({ link_id: "", script_id });
	...
	logger.log("Created a new script sharing link", { ip: request.clientIp, params: {user_id: request.session.user!.user_id, script_id, link_id} });
}
...
async function deleteShareLink(request: Request, response: Response, next: NextFunction) : Promise<void> {
	const link_id: string | null = request.params.link_id ? request.params.link_id : null;
	if (link_id === null) return next(new MissingQueryParams("link_id"));
	...
	await scriptShareLinkService.deleteShareLink(link_id);
	new APIResponse(201).send(response);

	logger.log("Deleted a script sharing link", { ip: request.clientIp, params: {user_id: request.session.user!.user_id, script_id: script.script_id, link_id} });
}
